...
void video_output_core1_run(void)
{
    ...
    // --- HSTX bit -> GPIO routing for the classic Pico DVI Sock (Wren6991 / Adafruit) ---
    // hstx_ctrl_hw->bit[i] drives GPIO(12+i). The sock wires:
    //   GP12/13 = D0 (+/-), GP14/15 = CLK (+/-), GP16/17 = D2 (+/-), GP18/19 = D1 (+/-)
    // '+' sits on the even GPIO (true signal), '-' on the odd GPIO (inverted).
#define HSTX_LANE_SEL(L)                                                                                                \
    ((uint32_t)((L) * 10) << HSTX_CTRL_BIT0_SEL_P_LSB | (uint32_t)((L) * 10 + 1) << HSTX_CTRL_BIT0_SEL_N_LSB)
    hstx_ctrl_hw->bit[0] = HSTX_LANE_SEL(0);                                   // GP12 D0+
    hstx_ctrl_hw->bit[1] = HSTX_LANE_SEL(0) | HSTX_CTRL_BIT0_INV_BITS;         // GP13 D0-
    hstx_ctrl_hw->bit[2] = HSTX_CTRL_BIT0_CLK_BITS;                            // GP14 CK+
    hstx_ctrl_hw->bit[3] = HSTX_CTRL_BIT0_CLK_BITS | HSTX_CTRL_BIT0_INV_BITS;  // GP15 CK-
    hstx_ctrl_hw->bit[4] = HSTX_LANE_SEL(2);                                   // GP16 D2+
    hstx_ctrl_hw->bit[5] = HSTX_LANE_SEL(2) | HSTX_CTRL_BIT0_INV_BITS;         // GP17 D2-
    hstx_ctrl_hw->bit[6] = HSTX_LANE_SEL(1);                                   // GP18 D1+
    hstx_ctrl_hw->bit[7] = HSTX_LANE_SEL(1) | HSTX_CTRL_BIT0_INV_BITS;         // GP19 D1-
#undef HSTX_LANE_SEL
